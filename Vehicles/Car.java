...
public class Car {
    ...
    public Car(String lic, int yr, Color cl, String mk, int pass) {
        setLicenseNumber(lic);
        setYear(yr);
        color = cl;
        setMake(mk);
        setPassengerSeats(pass);
    }
    ...
    public void setPassengerSeats(int pass) {
        if (pass < 1) {
            throw new IllegalArgumentException("The car must at least have one passenger seat.");
        }
        passengerSeats = pass;
    }
    ...
    public int hashCode() {
        ...
        sum += passengerSeats; // Adds passenger seats
